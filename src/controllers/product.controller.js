...
const { ProductValidity, Product, User} = require("../models");
...
const createProduct = async (req, res) => {
    try {
      ...
      let adjustedDescription = productDescription;
      if (productDescription.length < 200) {
        
      } else {
        adjustedDescription = productDescription.slice(0, 200);
       }

      const newProduct = new Product({
        productName,
        productDescription: adjustedDescription,
        maxActiveUsersCount,
...
const getProductsPricing = async (req, res) => {
    try {
...
const getProductsValidity = async (req, res) => {
    try {
...
const changeVisibilityProduct = async (req, res) => {
    try {
      const product = await Product.findById(req.params.productId);
